...
- Commit `76b9306` has now been pushed to GitHub `origin/main`, but the production Vercel domain still serves the previous asset bundle after polling. The GitHub leg is verified; the Vercel auto-deploy leg remains unverified without dashboard/CLI authentication.
- `npm run verify:vercel -- --production-url https://couple-calendar-sigma.vercel.app` now reproduces the Vercel freshness problem without credentials: local build assets are `/assets/index-DJ9XLkbq.js` and `/assets/index-DNerL9zV.css`, while production still serves `/assets/index-DhVCpYPU.js` and `/assets/index-3f7MBCgr.css`.
- The Vercel block was caused by the previous local Git author identity. After setting this repo's local author to `Observer-1013 <210874204+Observer-1013@users.noreply.github.com>` and pushing commit `10b5a17`, Vercel marked the deployment `Ready` and `Production`.
- Production freshness is now verified: `https://couple-calendar-sigma.vercel.app` serves the same asset bundle as the latest local build, and the production mobile auth smoke has 0 console errors.
